<template>
  <div class="min-w-full bg-gradient-to-tr from-pink-300 via-purple-300 to-indigo-400">
    <div class="container mx-auto">
      <div class="flex flex-col min-h-screen py-8 px-3 md:px-0">
        <div class="flex-none">
          <div class="flex items-stretch w-20 md:w-48 h-20 bg-white rounded-full shadow-xl ml-5 md:ml-0">
            <div class="self-center">
              <div class="w-20 h-20 p-4">
                <img alt="EffnerApp Logo" src="../assets/logo.png">
              </div>
            </div>
            <div class="invisible md:visible self-center font-medium text-xl pr-8">
              <p style="color: #3a3d3e;">EffnerApp</p>
            </div>
          </div>
        </div>

        <div class="flex-grow">
          <!--          md:pt-36 lg:pt-52 xl:pt-72 2xl:pt-80-->
          <div class="pt-24 pb-12 ml-4">
            <div
                class="block w-auto md:w-full break-words font-black text-2xl md:text-5xl">
              <p class="leading-tight" style="color: #3a3d3e;">Impressum der EffnerApp</p>
            </div>
            <div class="pt-2">
              <p class="leading-tight">(Stand: 22. Juni 2021)</p>
            </div>
          </div>
          <div class="pt-6 pb-12 grid md:grid-cols-2">
            <div class="space-y-8">
              <div class="ml-4 space-y-8">
                <table class="table-auto border-collapse border border-black">
                  <tbody>
                  <tr>
                    <td class="border border-black p-4 font-medium">Herausgeber:</td>
                    <td class="border border-black p-4">
                      <p>Josef-Effner-Gymnasium Dachau</p>
                      <p>Erich-Ollenhauer-Str. 12<br>85221 Dachau</p>
                      <br>
                      <p>Telefon: 08131/66647 0<br>Telefax: 08131/66647 11</p>
                      <br>
                      <p>E-Mail: <a class="underline" href="mailto:verwaltung@effner.de">verwaltung@effner.de</a></p>
                      <p>Internet: <a class="underline" href="https://effner.de">effner.de</a></p>
                    </td>
                  </tr>
                  <tr>
                    <td class="border border-black p-4 font-medium">Entwickler:</td>
                    <td class="border border-black p-4">
                      <p>Sebastian Müller</p>
                      <p>Telefon: +49 176 52327504</p>
                      <p>E-Mail: <a class="underline" href="mailto:sebi@effner.app">sebi@effner.app</a></p>
                      <br>
                      <p>Luis Bros</p>
                      <p>Telefon: +49 176 57952141</p>
                      <p>E-Mail: <a class="underline" href="mailto:luis@effner.app">luis@effner.app</a></p>
                    </td>
                  </tr>
                  <tr>
                    <td class="border border-black p-4 font-medium">Datenschutzbeauftragter:</td>
                    <td class="border border-black p-4">
                      <p>Jonathan Berger</p>
                      <p>Telefon: +49 1514 0192341</p>
                      <p>E-Mail: <a class="underline" href="mailto:datenschutz@effnerapp.de">datenschutz@effner.app</a></p>
                    </td>
                  </tr>
                  <tr>
                    <td class="border border-black p-4 font-medium">Verantwortlich für den Inhalt:</td>
                    <td class="border border-black p-4">
                      <p>Oberstudiendirektor Peter Mareis</p>
                    </td>
                  </tr>
                  </tbody>
                </table>
              </div>
            </div>
            <div>
              <div
                  class="block w-auto md:w-full break-words font-black text-2xl md:text-xl pb-4">
                <p class="leading-tight" style="color: #3a3d3e;">Haftungsausschluss</p>
              </div>

              <div class="block w-auto md:w-full break-words text-md">
                <p class="leading-tight" style="color: #3a3d3e;">
                <p>
                  Alle in dieser Anwendung bereitgestellten Informationen haben wir nach bestem Wissen und Gewissen
                  erarbeitet und geprüft. Eine Gewähr für die jederzeitige Aktualität, Richtigkeit,
                  Vollständigkeit und Verfügbarkeit der bereit gestellten Informationen können wir
                  allerdings nicht übernehmen. Ein Vertragsverhältnis mit den Nutzern des Internetangebots
                  kommt nicht zustande.
                </p>
                <br>
                <p>
                  Wir haften nicht für Schäden, die durch die Nutzung dieses Internetangebots entstehen.
                  Dieser Haftungsausschluss gilt nicht, soweit die Vorschriften des § 839 BGB (Haftung bei
                  Amtspflichtverletzung) einschlägig sind. Für etwaige Schäden, die beim Aufrufen oder
                  Herunterladen von Daten durch Schadsoftware oder der Installation oder Nutzung von Software verursacht
                  werden, übernehmen wir keine Haftung.
                </p>

              </div>
            </div>
          </div>
        </div>

        <div class="flex-none">
          <div class="flex justify-center">
            <Footer style="color: #3a3d3e;"/>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import Footer from "@/components/Footer";
export default {
  name: "Imprint",
  components: {Footer}
}
</script>

<style scoped>

</style>
